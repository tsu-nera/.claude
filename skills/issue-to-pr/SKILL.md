...
**重要: EnterPlanMode / ExitPlanMode は使用禁止。** 人間への確認は Phase 1（低品質Issue）・Phase 3.5（設計の不明点）・エラー時のみ。
PR 作成には承認を求めず、作成まで自走して事後に報告する（このスキルの成果物は PR。merge だけが人間の担当）。
...
**起動引数に `--autonomous` がある場合**（`/issue-to-merge` 配下）: 低品質でも確認せず、自分で方針を決めて続行する。
着手不能なほど要件が意味不明な場合のみ報告して中断。
...
**`--autonomous` 指定時**: このフェーズの AskUserQuestion は発火させず、自分で設計判断を下して Phase 4 へ進む
（`SPLIT_NEEDED` だけは例外として報告する）。
...
承認を求めず、コミット→PR作成まで実行する。**PR 作成後**に変更サマリと PR URL を報告する。
