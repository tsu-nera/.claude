...
テスト項目ごとにsonnetサブエージェントを **1つずつ逐次** 起動する。
前の tester の結果が返ってから次を起動する（同時に走らせない）。

型チェック（tsc）も vitest も CPU 全開で走る。他の worktree の agent も同じ 16 コアを
共有しているため、2つ以上を同時に走らせると全体が遅くなる。項目が独立していても並列化しない。
